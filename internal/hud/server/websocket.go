...
func (ws *WebsocketSubscriber) OnChange(ctx context.Context, s store.RStore, _ store.ChangeSummary) {
	...
	//     it eats up a lot of CPU/allocates a lot of memory on the backend, and
	//     triggers a full re-render on the frontend.
	...
	//
	// In future, we can maybe solve the backend problem more elegantly by replacing our JSON
	...
	// Solving the frontend problem probably requires breaking up the view and FE<->BE protocol.
