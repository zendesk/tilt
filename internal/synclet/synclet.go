...
func (s Synclet) UpdateContainer(
	...
	if err != nil {
		return errors.Wrapf(err, "error removing files while updating container %s: %v",
			containerId.ShortStr())
	}

	err = s.writeFiles(ctx, containerId, tarArchive)
	if err != nil {
		return errors.Wrapf(err, "error writing files while updating container %s: %v",
			containerId.ShortStr())
	}

	err = s.execCmds(ctx, containerId, commands)
	if err != nil {
		return errors.Wrapf(err, "error exec'ing commands while updating container %s: %v",
			containerId.ShortStr())
	}
	...
	if !hotReload {
		...
		if err != nil {
			return errors.Wrapf(err, "error restarting container %s: %v",
				containerId.ShortStr())
		}
	}
